...
function getComment(url,request){
  //console.log('getCommenturl: ' + url);

}
//returns all comments
function getComments(url,request){
 // console.log('getCommentsurl: ' + url);
}
...
function createComment(url,request){
  ...
  const response = {};
//ensure all requirements met before creating a comment
if( requestComment && requestComment.body&& database.articles[requestComment.articleId] && database.users[requestComment.username]){
  ...
  const comment ={
    ...
    //bind user to current comment id
    database.users[comment.username].commentIds.push(comment.id);
    
    //bind current article with current comment id
...
//updates a comment. Not sure if needed
function updateComment(url,request){
 const id = Number(url.split('/').filter(segment => segment)[1]);
 const savedComment = database.comments[id];
 const requestComment = request.body && request.body.comment;
 const response = {};
//check if there is an id and valid comment given
 if (!id || !requestComment || !request.body.comment.body) {
   response.status = 400;
...
function deleteComment(url,request){
  ...
  const response = {};
  if(savedComment){
    //deleting comment from article
    article = database.articles[savedComment.articleId];
    article.commentIds.splice(article.commentIds.indexOf(id),1);
    //deleting comment from user
    user = database.users[savedComment.username];
